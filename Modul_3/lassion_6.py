...
################ Решение ################
def date_structure_sum(date):
    total_sum = 0
    if isinstance(date, int):
        total_sum += date
    elif isinstance(date, str):
        total_sum += len(date)
    elif isinstance(date, (list, tuple, set)):
        for item in date:
            total_sum += date_structure_sum(item)
    elif isinstance(date, dict):
        for key, value in date.items():
            total_sum += date_structure_sum(key)
            total_sum += date_structure_sum(value)
...
result = date_structure_sum(data_structure)
print(result)
